...
public class ServletFarmacias extends HttpServlet {
   ...
   public void connectURL() throws IOException, JSONException{
	   HttpURLConnection connection = null;
		...
		try {
			...
			String request = "http://webfarmacia.azurewebsites.net/FarmaciaServer/api/farmacias";
   ...
   public JSONArray getResult_json() {
    	...
    	if (creado){
    		try {
    			for (int i=0; i<result_json.length(); i++){
    				...
    				if (!f_json_direccion.isNull("calle"))
    					d.setCalle(f_json_direccion.getString("calle"));
    				if (!f_json_direccion.isNull("numero"))
    					d.setNumero(f_json_direccion.getInt("numero"));
    				if (!f_json_direccion.isNull("letra"))
    					d.setLetra(f_json_direccion.getString("letra"));
    				if (!f_json_direccion.isNull("ciudad"))
    					d.setCiudad(f_json_direccion.getString("ciudad"));
    				if (!f_json_direccion.isNull("codigo_postal"))
    					d.setCodigo_postal(f_json_direccion.getInt("codigo_postal"));
    				if (!f_json_direccion.isNull("provincia"))
    					d.setProvincia(f_json_direccion.getString("provincia"));
    				if (!f_json_direccion.isNull("pais"))
    					d.setCalle(f_json_direccion.getString("pais"));
    					
    				f.setDireccion(d);
    				
    				// horarios
    				f.setHorario_abrir(new Date(f_json.getLong("horario_abrir")));
    				f.setHorario_cerrar(new Date(f_json.getLong("horario_cerrar")));
	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		// TODO Auto-generated method stub
				...
				if (accion.equals("new")){		
    				...
    				try {
						farmacia.setHorario_abrir(df.parse(hora_apertura));
						farmacia.setHorario_cerrar(df.parse(hora_cierre));
					...
					response.sendRedirect("http://webfarmacia.azurewebsites.net/FarmaciaWeb/farmacias.jsp");
				}
	}
	...
	private void sendFarmaciaPOST(Farmacia farmacia) throws IOException {
		...
		try {
			  //constants
			  URL url = new URL("http://webfarmacia.azurewebsites.net/FarmaciaServer/api/farmacias");
	...
	private void deleteFarmacia(String cif_farmacia) throws IOException {
		...
		try {
			  //constants
			  URL url = new URL("http://webfarmacia.azurewebsites.net/FarmaciaServer/api/farmacias/"+cif_farmacia);
			  conn = (HttpURLConnection) url.openConnection();
			  conn.setReadTimeout( 10000 /*milliseconds*/ );
			  conn.setConnectTimeout( 15000 /* milliseconds */ );
