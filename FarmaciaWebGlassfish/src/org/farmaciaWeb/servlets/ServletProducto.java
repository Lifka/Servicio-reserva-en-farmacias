   ...
   public void connectURL() throws IOException, JSONException{
	   HttpURLConnection connection = null;
		...
		try {
			...
			String request = "http://localhost:8080/Farmacia/api/productos";
    		...
    		try {
    			for (int i=0; i<result_json.length(); i++){
    				...
    				Producto p = new Producto(pr_json.getInt("id"), pr_json.getString("nombre"), pr_json.getString("descripcion"),
    						pr_json.getDouble("precio_sin_iva"), pr_json.getString("f_creacion"), pr_json.getString("f_caducidad"),
	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		if (accion.equals("new")){		
			...
			String dep = request.getParameter("dep_prod");
			 ...
			 producto = new Producto(-1,nombre,descripcion,Double.parseDouble(precio),null,null, Departamento.valueOf(dep),Double.parseDouble(iva)/100+1);
			...
			response.sendRedirect("http://localhost:8080/FarmaciaWeb/productos.jsp");
		}
	...
	private void deleteProducto(String id) throws IOException {
		...
		try {
			  //constants
			  URL url = new URL("http://localhost:8080/Farmacia/api/productos/"+id);
	...
	private void sendProductoPOST(Producto p) throws IOException{
		...
		try {
			  //constants
			  URL url = new URL("http://localhost:8080/Farmacia/api/productos");
